...
RSpec.describe ItemRepository do
    ...
    #     all_by_price_range = items.find_all_by_price_range(BigDecimal('1300.00')..BigDecimal('1350.00'))

    #     expect(all_by_price_range).to be_an(Array)
    ...
    it "#find_all_by_price_range" do
        price_range = (700.00..1350)
        items_in_range = @itemrepository.find_all_by_price_range(price_range)
        expect(items_in_range).to be_an(Array)
        expect(items_in_range.first).to be_an_instance_of Item
        expect(items_in_range.first.name).to eq("510+ RealPush Icon Set")
    end
